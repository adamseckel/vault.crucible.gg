  ...
  return {
    insertOrUpdateUserAndTrackVisit({membershipId, locale, blizzardDisplayName = 'None', displayName, psnDisplayName = 'None', xboxDisplayName = 'None'}) {
      return Promise.all([
        ...
        db.child('sessions-by-bungie-id').child(membershipId).push(Date.now())
      ]);
    },

    insertOrUpdateCharacters(bungieID, charactersByID) {
      return db.child('characters-by-bungie-id').child(bungieID).update(charactersByID);
    },

    insertOrUpdateItems(bungieID, itemsByID) {
      return db.child('items-by-bungie-id').child(bungieID).update(itemsByID);
    },

    trackTransferByBungieID(bungieID, tracking) {
      return db.child('transfers-by-bungie-id').child(bungieID).push(tracking);      
    },

...
function reduceToBuckets(buckets, item) {
  if (
    ...
    buckets[item.inventory.bucketTypeHash].items[item.characterID]
  ) {
...
function calculateRowHeight(bucketItems, vaultColumns) {
  return Object.keys(bucketItems)
    .map(characterID => {
  ...
  return {
    ...
    getItems(clientWidth, characters, membershipId, manifest, bucketDefinitions, statsDefinitions) {
      const requests = characters.map(character => {
        ...
        return getBungieRequest().then(bungieRequest =>
          bungieRequest
            .getCharacterById(characterId, membershipId)
            .then(({ character, inventory, equipment, itemComponents }) => {
              return {
                character,
                inventory,
                equipment,
      ...
      requests.push(
        getBungieRequest().then(bungieRequest =>
          bungieRequest
            .getProfileInventory(membershipId)
            .then(({ profileInventory, itemComponents }) => {
              return {

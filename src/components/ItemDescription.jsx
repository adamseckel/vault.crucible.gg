...
export default(props) => {
  ...
  return <div>
    {props.item.instance && props.item.instance.primaryStat && props.item.instance.primaryStat.value > 0 && <Row justify='start' css={`margin-bottom: 8px`}>
      ...
      <Text css={`color: ${getColor(props.primaryStatType)} !important;`} bold size={4}>{props.item.instance ? props.item.instance.primaryStat.value : 0}</Text>
      <Text css={`color: ${getColor(props.primaryStatType)} !important; margin-left: 8px;`}>{props.item.instance && props.primaryStatType}</Text>
    </Row>}

...
export default ({ perksDefinitions, perks, className, style }) => {
    ...
    return perkList
      ...
      .map(perk => {
        ...
        return {
          ...
          isMod:
            perksDefinitions[perk.perkHash].displayProperties.name.split(' ').includes('Mod') ||
            perksDefinitions[perk.perkHash].displayProperties.name.split(' ').includes('Module'),
        };
      })
      .map(perk => (
        <PerkRow justify="start" align="start" key={perk.perkHash}>
          {perk.iconPath && (
            <PerkIcon mod={perk.isMod} src={`https://bungie.net${perk.iconPath}`} alt={perk.name} />
          )}
  ...
  return <div {...{ className, style }}>{perks && mapPerks(perks)}</div>;
};

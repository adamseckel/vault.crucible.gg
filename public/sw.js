self.addEventListener('install', function(event) {
  event.waitUntil(
    caches.open('fonts').then(function(cache) {
      return cache.addAll(['https://fonts.googleapis.com/icon?family=Material+Icons']);
    }),
  );
});
      ...
      caches.match(event.request).then(function(response) {
        ...
        return (
          ...
          fetch(request).then(response => {
            return caches.open('icons').then(cache => {
    ...
    event.respondWith(
      caches.match(event.request).then(function(response) {
        ...
        return (
          response ||
          fetch(request).then(response => {
            return caches.open('BungieInventoryItemRequests').then(cache => {
              cache.put(event.request, response.clone());
              return response;
            });
          })
    ...
    event.respondWith(
      caches.match(event.request).then(function(response) {
        ...
        return (
          response ||
          fetch(request).then(response => {
            return caches.open('JSONManifests').then(cache => {
              cache.put(event.request, response.clone());
              return response;
            });
          })
